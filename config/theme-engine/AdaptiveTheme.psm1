...
function New-YasbThemeCss {
    param($Theme)$r=$Theme.roles;$barOpacity=.78;$popupOpacity=.90;if($Theme.PSObject.Properties.Name-contains'opacity'){$barOpacity=[double]$Theme.opacity.bar;$popupOpacity=[double]$Theme.opacity.popup};$bg=Convert-HexToRgba $r.background $barOpacity;$popup=Convert-HexToRgba $r.background $popupOpacity;$surface=Convert-HexToRgba $r.surface ([Math]::Min(.92,$barOpacity+.04));$soft=Convert-HexToRgba $r.accentSoft .42;$hover=Convert-HexToRgba $r.hover .70;$active=Convert-HexToRgba $r.active .86
@"
